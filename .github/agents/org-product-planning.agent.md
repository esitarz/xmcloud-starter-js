...
tools: [search/codebase, read/readFile, edit/createFile, edit/editFiles, execute/runInTerminal, execute/getTerminalOutput, web/fetch, atlassian/*]
---
...
Then follow [git-branch-and-save.prompt.md](.github/prompts/git-branch-and-save.prompt.md) to save the file at `specs/proposal-{slug}.md` on branch `specs/proposal-{slug}`. Apply slug rules from [slug-generation.prompt.md](.github/prompts/slug-generation.prompt.md) to the slug before saving.
...
## Phase 2b: Create Jira Ticket

Follow [jira-ticket-creation.prompt.md](.github/prompts/jira-ticket-creation.prompt.md) using the spec content. For the copy-paste summary, include the spec path (`specs/proposal-{slug}.md`) at the bottom.

**If the PO hasn't saved yet**, complete Phase 2 first, then create the ticket.

---

## Phase 3: Finalize with Ticket ID

When the PO returns with the ticket ID, follow the Finalize phase in [spec-lifecycle.prompt.md](.github/prompts/spec-lifecycle.prompt.md). The spec body carries through unchanged — all requirements, examples, edge cases, open questions, and notes stay intact.

After pushing:

> "Done! Spec finalized at `specs/{TICKET-ID}-{slug}.md`.
>
> **Next steps:**
