...
export const Default: React.FC<ProductListingProps> = ({ fields, isPageEditing }) => {
  const isEditing = Boolean(isPageEditing);
  ...
  if (!datasource) {
    return (
      <section className="space-y-3 rounded-lg border border-dashed p-4 text-sm text-amber-700" data-component="ProductListing">
        ...
        {!isEditing && <OrderCloudProductList title="Live OrderCloud products" compact />}
      </section>
    );
  }
  ...
  return (
    <section className="space-y-4" data-component="ProductListing">
      {hasTextValue(datasource.title?.jsonValue) && (
        <Text field={datasource.title.jsonValue} tag="h2" className="text-2xl font-semibold" />
      )}
      ...
      {hasLinkValue(datasource.viewAllLink?.jsonValue) && (
        <div>
          <Link field={datasource.viewAllLink.jsonValue} className="text-sm font-semibold underline" />
        </div>
      )}
    </section>
  );
